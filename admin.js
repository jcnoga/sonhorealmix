...
function openProductModal(p = null) {
  ...
  // Reset file inputs
  ['pf-photo1-file','pf-photo2-file','pf-video-file'].forEach(id => {
    const el = qs(`#${id}`);
    if (el) el.value = '';
  });
...
async function saveProduct() {
  ...
  try {
    ...
    if (videoFile)  video  = await uploadFile(videoFile, `products/${Date.now()}_video_${videoFile.name}`);
